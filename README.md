...
Relayer wallet funding: the Support (♡) modal always shows the address (tap to copy) and P2P/exchange links (Kraken, Binance P2P, Noones, P2PMoney, Allbridge). Optionally, `PRIVY_APP_ID` (+ `PRIVY_CLIENT_ID`, from the [Privy dashboard](https://dashboard.privy.io/)) enables Privy's real login modal (email or "Continue with a wallet") for sending preset amounts (0.5/1/5 CELO) directly — hidden automatically when unset. This uses the real `@privy-io/react-auth` package ([docs](https://docs.privy.io/)), the same one used by the Choco for MiniPay project; Privy has no CDN/script-tag build, so the widget is a small React island pre-bundled with esbuild and committed as `privy-widget.bundle.js` (loaded lazily, only when the Privy section is shown). After changing `src/privy-widget.jsx`, run `npm run build:privy` and commit the updated bundle — Vercel does not run a build step for it.
